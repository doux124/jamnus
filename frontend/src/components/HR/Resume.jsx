  ...
  const submitFile = async (selectedFile) => {
    ...
    try {
      ...
      const lawMessage = { 
        ...
        text: `**Legal Analysis**: ${response.data.pros}`, 
        sender: 'bot' 
      };
      
      const recMessage = { 
        id: messages.length + 4, 
        text: `**Recommendation**: ${response.data.cons}`, 
        sender: 'bot' 
      };
    ...
    <div className="flex flex-col w-screen h-screen overflow-hidden bg-gray-100">
      ...
      <header className="bg-white border-b shadow-sm py-4 px-4">
        <div className="w-full max-w-6xl mx-auto flex items-center justify-between">
          <div className="text-xl font-bold text-gray-800">Resume Analysis</div>
          <div className="flex items-center gap-4">
            <Link to="/" className="text-blue-500 hover:underline">Home</Link> 
            <div className="text-sm text-gray-500">Analyzes candidates</div>
          </div>
        </div>
      </header>

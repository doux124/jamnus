  ...
  const submitText = async () => {
    ...
    try {
      ...
      const outputMessage1 = { 
        ...
        text: `${response.data.output1}`, 
        sender: 'bot' 
      };
      const outputMessage2 = { 
        id: messages.length + 3, 
        text: `${response.data.output2}`, 
        sender: 'bot' 
      };
      const outputMessage3 = { 
        id: messages.length + 3, 
        text: `${response.data.output3}`, 
        sender: 'bot' 
      };
      const outputMessage4 = { 
        id: messages.length + 3, 
        text: `${response.data.output4}`, 
        sender: 'bot' 
      };
      const outputMessage5 = { 
        id: messages.length + 3, 
        text: `${response.data.output5}`, 
        sender: 'bot' 
      };
      const outputMessage6 = { 
        id: messages.length + 3, 
        text: `${response.data.output6}`, 
        sender: 'bot' 
      };
    ...
    <div className="flex flex-col w-screen h-screen overflow-hidden bg-gray-100">
      ...
      <header className="bg-white border-b shadow-sm py-4 px-4">
        <div className="w-full max-w-6xl mx-auto flex items-center justify-between">
          <div className="text-xl font-bold text-gray-800">Pre-Coaching Evaluation AI</div>
